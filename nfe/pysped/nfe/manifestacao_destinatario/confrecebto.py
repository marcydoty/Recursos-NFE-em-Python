...
from nfe.pysped.nfe.manual_401 import cancnfe_evento as Evento
import os
...
class InfEventoEnviado(XMLNFe):
    def __init__(self):
        ...
        self.nProt    = TagCaracter(nome=u'nProt'    , codigo=u'HP20' , tamanho=[15, 15, 15], raiz=u'//envEvento/infEvento/detEvento', obrigatorio=False)
        self.xJust = TagCaracter(nome=u'xJust'  , codigo=u'HP21', tamanho=[15, 255]   , raiz=u'//envEvento/infEvento/detEvento', obrigatorio=False)
...
class  EnvConfRecebto(XMLNFe):
    def __init__(self):
        super(EnvConfRecebto, self).__init__()
...
class InfEventoRecebido(XMLNFe):
    def __init__(self):
        ...
        self.tpAmb = TagInteiro(nome=u'tpAmb'   , codigo=u'HR13', tamanho=[ 1,  1, 1] , raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False)
        self.verAplic = TagCaracter(nome=u'verAplic', codigo=u'HR14' , tamanho=[1, 20]     , raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False)
        self.cOrgao  = TagInteiro(nome=u'cOrgao'      , codigo=u'HR15', tamanho=[2, 2,2]   , raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False )
        self.cStat    = TagCaracter(nome=u'cStat'    , codigo=u'HR16' , tamanho=[3, 3, 3]   , raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False)
        self.xMotivo  = TagCaracter(nome=u'xMotivo' , codigo=u'HR17' , tamanho=[1, 255]    , raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False)
        ...
        self.tpEvento = TagCaracter(nome=u'tpEvento'   , codigo=u'HR19', tamanho=[6, 6, 6], raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False)
        self.xEvento  = TagCaracter(nome=u'xEvento'   , codigo=u'HR20' , tamanho=[ 5, 60]   , raiz=u'//retEnvEvento/retEvento/infEvento', obrigatorio=False)
        self.nSeqEvento = TagInteiro(nome=u'nSeqEvento'   , codigo=u'HR21', tamanho=[2,2], raiz=u'//retEnvEvento/retEvento/infEvento',obrigatorio=False, valor=1)
        ...
        self.dhRegEvento = TagCaracter(nome=u'dhRegEvento', codigo=u'HR25' ,  tamanho=[ 0, 30],raiz=u'//retEnvEvento/retEvento/infEvento', obrigatorio=False)
...
class RetEnvConfRecebto(XMLNFe):
    def __init__(self):
        super(RetEnvConfRecebto, self).__init__()
        ...
        self.ret_evento_versao = TagDecimal(nome=u'retEvento', codigo=u'HR09', raiz=u'//retEnvEvento', valor=u'1.00',  propriedade=u'versao', obrigatorio=False)
        self.infEvento   = InfEventoRecebido()
        self.Signature = Signature()
        self.caminho_esquema = os.path.join(DIRNAME, u'schema/', ESQUEMA_ATUAL + u'/')
        self.arquivo_esquema = u'retEnvConfRecebto_v1.00.xsd'
...
class ProcEventoNFe(XMLNFe):
    def __init__(self):
        ...
        self.envEvento = EnvConfRecebto()
        self.retEnvEvento = RetEnvConfRecebto()
        self.caminho_esquema = os.path.join(DIRNAME, u'schema', ESQUEMA_ATUAL + u'/')
        self.arquivo_esquema = u'procConfRecebtoNFe_v1.00.xsd'
    ...
    def get_xml(self):
        ...
        if self.retEnvEvento.ret_evento_versao.xml:
